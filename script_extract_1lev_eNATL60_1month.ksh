	...
	case $month in
		...
		mm=$(printf "%02d" $monthi)
    		...
    		for var in $VAR; do
      			...
      			stdir=/lus/store/CT1/hmg2840/brodeau/${CONFIG}/${CONFIG}-${CASEi}-S
			...
			for file in $(ls $stdir/*/${CONFIG}-${CASEi}_${FREQ}_*_${filetyp}_${year}${mm}${dd}-${year}${mm}${dd}.nc); do
				...
				if [ ! -f  $fileo ]; then 
					echo $fileo
					/lus/home/CT1/ige2071/aalbert/.conda/envs/nco/bin/ncks -O -F $coordi -d $dep,$LEV,$LEV -v ${var} $file $fileo
				fi
